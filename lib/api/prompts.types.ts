...
export interface PromptResponse {
  ...
  userId: string;
  /**
   * The API sends `responseText`. There was also an optional `content` here
   * once; both being optional meant reading the wrong one compiled cleanly and
   * silently rendered an empty string, so only the real field is declared now.
   */
  responseText: string;
  likesCount: number;
