...
import { INVITE_STEP } from "@/lib/launch";
...
import { useCompleteAmbassadorOnboardingMutation, useListAllMarketsQuery } from "@/features/api/apiSlice";
...
export function SchoolStep() {
  ...
  const { eyebrow, titleLines, agreement, cta } = INVITE_STEP;
  ...
  const { data: markets } = useListAllMarketsQuery();
  ...
  const selectSchool = (school: School) => {
    ...
    // A school belongs to exactly one market, so prefill it rather than making
    // them answer a question we already know the answer to.
    if (school.marketId) {
      setMarketId(school.marketId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!marketId) errors.market = "Please select your market.";
    ...
    if (errors.market) return marketRef.current?.focus();
    ...
    try {
      ...
      sessionStorage.setItem("onboarding_market_id", marketId);
    ...
    <section className="launch-step launch-step--personalize">
      <div className="launch-step-inner launch-step-inner--personalize">
        <div className="personalize-content">
          ...
          <h1 className="launch-title personalize-title font-canela onboarding-heading">
          ...
          <form onSubmit={handleSubmit} onKeyDown={handleFormKeyDown} className="launch-form" noValidate>
            ...
            <div>
              <label className="launch-field-label" htmlFor="launch-market">
                Market
              </label>
              <div className="launch-select-wrap">
                <select
                  ref={marketRef}
                  id="launch-market"
                  className={`launch-field-input launch-field-select${marketId ? "" : " is-placeholder"}${fieldErrors.market ? " is-invalid" : ""}`}
                  value={marketId}
                  aria-invalid={Boolean(fieldErrors.market)}
                  aria-describedby={fieldErrors.market ? "launch-market-error" : undefined}
                  onChange={(e) => {
              ...
              {fieldErrors.market && (
                <p className="launch-field-error" id="launch-market-error" role="alert">
                  {fieldErrors.market}
                </p>
              )}
            </div>
