...
export function VerifyEmailStep() {
  ...
  const handleResend = async () => {
    ...
    setResendError(null);
    try {
      const resentInvite = await resendInvite(token).unwrap();
      if (resentInvite.token) {
        sessionStorage.setItem("ambassador_onboarding_token", resentInvite.token);
        setToken(resentInvite.token);
      }
      setResent(true);
    } catch (err) {
  ...
  return shell(
    <>
      ...
      <div className="verify-links">
        ...
        <Link href={useDifferentEmail.href} className="verify-link">
