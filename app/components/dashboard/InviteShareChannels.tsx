...
/* "chip" wraps each icon in a bordered circle (invite page); "flat" drops the
   circle and sets the icon straight on the card (ambassador overview). */
type ShareChannelVariant = "chip" | "flat";

interface InviteShareChannelsProps {
  shareUrl: string;
  shareTitle?: string;
  onCopied?: () => void;
  variant?: ShareChannelVariant;
}
  ...
  if (variant === "flat") {
    return (
      <div className="flex flex-col items-center gap-3.5 cursor-pointer group">
        <div className="relative w-5 h-5 md:w-6.5 md:h-6.5 group-hover:scale-105 transition-transform duration-200">
          <Image src={icon} alt="" fill className="object-contain" aria-hidden />
        </div>
        <span className="text-[11px] md:text-[13px] font-sans font-medium text-[#4a3429] group-hover:text-black transition-colors text-center whitespace-nowrap">
          {copied ? "Copied!" : label}
        </span>
      </div>
    );
      ...
      {SHARE_CHANNELS.map((channel) => {
        if (channel.id === "whatsapp") {
          return (
            ...
            >
              <ShareChannelVisual label={channel.label} icon={channel.icon} variant={variant} />
            </WhatsappShareButton>
          );
        }
