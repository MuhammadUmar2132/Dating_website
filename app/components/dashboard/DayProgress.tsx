...
export function DayProgress({ dayNumber, totalDays, label }: DayProgressProps) {
  if (!dayNumber) {
    return (
      <span className="text-[12px] md:text-[14px] font-lato font-bold text-neutral-500 uppercase tracking-[0.14em] whitespace-nowrap">
        {label}
      </span>
    );
  }
  ...
  return (
    <div
      ...
      aria-label={label}
    >
      <span className="text-[12px] md:text-[14px] font-lato font-bold text-neutral-500 uppercase tracking-[0.14em] whitespace-nowrap tabular-nums">
        Day {dayNumber}
        ...
        {Array.from({ length: totalDays }, (_, i) => {
          ...
          return (
            <span
              key={day}
              className={
                "h-[4px] rounded-full transition-colors " +
                (current
                  ? "w-5 md:w-6 bg-[#e8a588]"
                  : done
                    ? "w-2.5 md:w-3 bg-neutral-800"
                    : "w-2.5 md:w-3 bg-neutral-300")
              }
            />
          );
        })}
