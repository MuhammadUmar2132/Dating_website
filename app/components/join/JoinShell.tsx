...
/** The current artboards have no step indicator. Flip to re-enable. */
const SHOW_STEP_DOTS = false;
...
type Props = {
  ...
  /** CTA wording. The first step reads "Join the waitlist" in the artboards. */
  ctaLabel?: string;
    ...
    >
      <header className="jn-top">
        <Link href="/" className="jn-brand" aria-label="Bubba — home">
          {/* The same heavier cut the intro uses — logo.png is the light
              drawing and read thinner than the wordmark on the intro. */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={BUBBA_BRAND.wordmarkBlack} alt="Bubba" />
        </Link>
        {/* The plain artboards carry no step indicator — the brand mark sits
            alone above a centred column. Kept behind the flag so re-adding the
            dotted variant is a one-line change. */}
        {SHOW_STEP_DOTS ? <JoinDots current={index} /> : null}
      </header>
