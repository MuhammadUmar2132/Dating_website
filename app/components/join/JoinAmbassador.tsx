...
export function JoinAmbassador() {
  ...
  /* Scoped to the campus the signup just picked, falling back to their market
     so the roster is people they could plausibly know. */
  ...
  const ambassadors = data?.slice(0, 4) ?? [];
  const isEmpty = !isLoading && ambassadors.length === 0;

  return (
    <JoinShell
      slug="ambassador"
      art="/bubba/faq-cyclist.png"
      artSide="right"
      skip={{
    ...
    >
      {isLoading ? (
        ...
        <p className="jn-amb-empty">
          No ambassadors are listed for your campus yet. Continue without one.
        </p>
      ) : (
        <ul className="jn-amb">
          {ambassadors.map((a) => {
