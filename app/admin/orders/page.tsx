...
/* Mirrors VALID_TRANSITIONS in the API's order.service.js. Offering a status the
   API will refuse just produces "Invalid order status transition", so the editor
   lists only the moves that are actually legal from where the order is now. */
...
function OrderDetail({
  ...
  const nextStatuses = NEXT_STATUSES[order.status.toUpperCase()] ?? [];
  const [status, setStatus] = useState(nextStatuses[0] ?? order.status);
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="border border-neutral-200/60 rounded-[12px] p-4 bg-neutral-50/50 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="o-status" className={labelClass}>
            Order Status <span className="font-normal normal-case text-neutral-400">(currently {order.status})</span>
          </label>
          {nextStatuses.length ? (
            ...
            <p className="font-lato text-[12px] font-medium text-neutral-500">
              A {order.status.toLowerCase()} order cannot move to another status.
            </p>
          )}
          {order.status.toUpperCase() === "CONFIRMED" ? (
            <p className="font-lato text-[11px] font-medium text-neutral-400">
              Fulfilment runs in order: Processing, then Shipped, then Delivered.
            </p>
          ) : null}
        </div>
  ...
  return (
    <main className="flex-1 flex flex-col gap-6 md:gap-8 min-w-0">
      ...
      <DataTable
        rows={filteredOrders}
        columns={columns}
        gridCols="grid-cols-[1.6fr_1.4fr_0.8fr_0.9fr_1.3fr_0.9fr_0.6fr]"
        minWidth="880px"
        getRowKey={(o) => o.id}
        loading={loading}
        pagination={{
