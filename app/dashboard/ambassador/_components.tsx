...
export function MetricCard({ label, value, note }: { label: string; value: ReactNode; note?: string }) {
  return (
    <div className="rounded-[10px] border border-neutral-200/70 bg-[#fbfbf9] p-4">
      <p className="font-sfpro text-[10px] font-bold uppercase tracking-[0.1em] text-neutral-500">{label}</p>
      <p className="mt-1.5 font-minionvariable text-[20px] font-bold text-neutral-900">{value}</p>
      {note ? <p className="mt-1 font-lato text-[11px] font-semibold text-neutral-400">{note}</p> : null}
    </div>
  );
}
...
export function Panel({ title, children, action }: { title: string; children: ReactNode; action?: { href: string; label: string } }) {
  return (
    <section className="rounded-[12px] border border-neutral-200/60 bg-[#fbfbf9] p-4 md:p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className="font-sfpro text-[11px] md:text-[12px] font-bold uppercase tracking-[0.14em] text-[#402b23]">
          {title}
        </h2>
        {action ? (
